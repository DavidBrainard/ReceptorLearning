...
(defn -main [& args]
  (if (not= (count args) 3)
    (die "Arguments required: plan-id number-of-workers worker-id")
    (let [[plan-id-str number-of-workers-str worker-id-str] args
      ...
      (simulate-some-retinas plan-id number-of-workers worker-id :verbose true)))
  (System/exit 0))
